...
export default function Users() {
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    const res = ControlCopyDB.addUser({
      nome,
      email,
      whatsapp,
      telegram,
      ...
      status,
      link_cadastro_utilizado: `https://iqoption.net/lp/mobile-partner-pwa/?aff=417345&aff_model=revenue${indicadorId ? '&afftrack=' + (indicators.find(i=>i.id===indicadorId)?.codigo_interno || '') : ''}`,
  ...
  const handleUpdateBalanceSubmit = (e: React.FormEvent) => {
    ...
    const updateRes = ControlCopyDB.recordBalanceUpdate(selectedUser.id, Number(newBalance));
    ...
    if (updateRes.success) {
      ...
      if (shouldBill && diffProfit > 0) {
        ...
        const bill = ControlCopyDB.billUserCycle(selectedUser.id, diffProfit);
        if (bill) {
      ...
      const updUser = ControlCopyDB.getUsers().find(x => x.id === selectedUser.id);
            ...
            <tbody>
              {filteredUsers.length > 0 ? (
                filteredUsers.map((user) => {
                  ...
                  const isDueToday = user.proxima_cobranca === dateUtils.todayStr();

                  return (
                    <tr key={user.id} className="border-b border-zinc-50 hover:bg-zinc-50/25 transition-all">
                      {/* Name & ID */}
                      ...
                      <td className="py-3.5 px-4">
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-extrabold border uppercase font-mono tracking-tight ${
                          user.plano === 'SEMANAL' 
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex flex-col">
                          ...
                          <span className={`text-[10px] font-bold ${isDueToday ? 'text-[#FF5500] animate-pulse font-extrabold' : 'text-zinc-400 font-mono'}`}>
                            {isDueToday ? '⏰ Vence Hoje!' : `${dateUtils.daysUntil(user.proxima_cobranca)} dias restantes`}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="py-3.5 px-4">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold font-mono border ${
                          user.status === 'Ativo' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                          user.status === 'Pendente' ? 'bg-orange-50 text-orange-500 border-orange-100' :
                          user.status === 'Pausado' ? 'bg-amber-50 text-amber-600 border-amber-100' :
        ...
        {isDetailsOpen && selectedUser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-zinc-50 p-4 rounded-2xl mb-5 text-[11px] font-semibold text-zinc-650">
                <div>
